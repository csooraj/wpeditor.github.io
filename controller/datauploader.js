...
function UploadSplash(){
  ...
  if((backgroundColor!=='')&&(titleSize!=='')&&(titleColor!=='')&&(title!=='')&&(layouttype!=='')){
    ...
    if(file===undefined){
      var data = {"AppName":appname, "SplashBgColor": backgroundColor, "type":layouttype,  "Splashtitle": title, "SplashtitleSize":titleSize, "SplashtitleColor": titleColor};
    }else{
      var data = {"AppName":appname, "SplashBgColor": backgroundColor, "type":layouttype,  "SplashlogoUrl":file.name,  "Splashtitle": title, "SplashtitleSize":titleSize, "SplashtitleColor": titleColor};
    }
...
function UploadLogin(){
  ...
  if((login_labelColor!=='')&&(login_labelSize!=='')&&(login_buttonColor!=='')&&(login_buttonTextSize!=='')&&(login_inputBgColor!=='')){
    ...
    if((file1===undefined)&&(file2===undefined)){
      var data = {"AppName":appname, "InputBgColor":login_inputBgColor,  "LoginLabelColor":login_labelColor, "LoginLabelSize":login_labelSize, "LoginButtonColor":login_buttonColor, "LoginButtonTextSize":login_buttonTextSize};
    }else if(file1===undefined){
      var data = {"AppName":appname, "InputBgColor":login_inputBgColor, "LoginLogo":file2.name, "LoginLabelColor":login_labelColor, "LoginLabelSize":login_labelSize, "LoginButtonColor":login_buttonColor, "LoginButtonTextSize":login_buttonTextSize};
    }else if(file2===undefined){
      var data = {"AppName":appname, "InputBgColor":login_inputBgColor, "LoginBgUrl":file1.name, "LoginLabelColor":login_labelColor, "LoginLabelSize":login_labelSize, "LoginButtonColor":login_buttonColor, "LoginButtonTextSize":login_buttonTextSize};
    }else{
      var data = {"AppName":appname, "InputBgColor":login_inputBgColor, "LoginBgUrl":file1.name, "LoginLogo":file2.name, "LoginLabelColor":login_labelColor, "LoginLabelSize":login_labelSize, "LoginButtonColor":login_buttonColor, "LoginButtonTextSize":login_buttonTextSize};
    }
    firebase.database().ref("Test").update(data, function(error) {
         if (error !== null) {
             alert("Some Error Occured Try Again");
...
function ReloadPreview(type){
  var iframe = document.getElementById('mine');
  if(type===1){
    iframe.src = "https://appetize.io/embed/tw35jqdej5f8xqbf8qgy817n5g?device=nexus5&scale=75&orientation=portrait&osVersion=7.0";
  }else{
...
function UploadTestAssets(type){
  ...
  if (file) {
    var name = file.name;
    var params = {Key:"Assets/"+name, ContentType: file.type, Body: file};
    bucket.upload(params).on('httpUploadProgress', function(evt) {
      console.log("Uploaded :: " + parseInt((evt.loaded * 100) / evt.total)+'%');
    }).send(function(err, data) {
      console.log(err);
